...
# p1
> /tmp/gitlab-assignments-checker-4-1.json
PAGES=$(curl -i -s -H "PRIVATE-TOKEN: $privatetoken" "https://gitlab.com/api/v4/issues?scope=all&labels=Dimi%3A%3AIn%20Progress%20P1&state=opened&per_page=100" | grep -Fi X-Total-Pages | awk '/X-Total-Pages/ { print $2 }' | tr -d '\r');
for i in $(seq 1 $PAGES); do
  curl -s -L -H "PRIVATE-TOKEN: $privatetoken" "https://gitlab.com/api/v4/issues?scope=all&labels=Dimi%3A%3AIn%20Progress%20P1&state=opened&per_page=100&page=$i" >> /tmp/gitlab-assignments-checker-4-1.json;
done

# p2
> /tmp/gitlab-assignments-checker-5-1.json
PAGES=$(curl -i -s -H "PRIVATE-TOKEN: $privatetoken" "https://gitlab.com/api/v4/issues?scope=all&labels=Dimi%3A%3AIn%20Progress%20P2&state=opened&per_page=100" | grep -Fi X-Total-Pages | awk '/X-Total-Pages/ { print $2 }' | tr -d '\r');
for i in $(seq 1 $PAGES); do
  curl -s -L -H "PRIVATE-TOKEN: $privatetoken" "https://gitlab.com/api/v4/issues?scope=all&labels=Dimi%3A%3AIn%20Progress%20P2&state=opened&per_page=100&page=$i" >> /tmp/gitlab-assignments-checker-5-1.json;
done

# standby
> /tmp/gitlab-assignments-checker-6-1.json
PAGES=$(curl -i -s -H "PRIVATE-TOKEN: $privatetoken" "https://gitlab.com/api/v4/issues?scope=all&labels=Dimi%3A%3AStandby&state=opened&per_page=100" | grep -Fi X-Total-Pages | awk '/X-Total-Pages/ { print $2 }' | tr -d '\r');
for i in $(seq 1 $PAGES); do
  curl -s -L -H "PRIVATE-TOKEN: $privatetoken" "https://gitlab.com/api/v4/issues?scope=all&labels=Dimi%3A%3AStandby&state=opened&per_page=100&page=$i" >> /tmp/gitlab-assignments-checker-6-1.json;
done
...
echo "---";
file=/tmp/gitlab-assignments-checker-1-p1.txt
> $file
echo "Dimi P1 | bash=/Users/dimitrie/.dotfiles/bin/openlist param1=$file terminal=false color=$headercolor";
while read -r iid
      read -r labels
      read -r title
      read -r web_url; do
...
| href=$web_url font=$monofont size=$monosize";
echo $web_url >> $file;
done < <(jq -rc '.[] | .iid,.labels,.title,.web_url' < /tmp/gitlab-assignments-checker-4-1.json);

echo "---";
file=/tmp/gitlab-assignments-checker-1-p2.txt
> $file
echo "Dimi P2 | bash=/Users/dimitrie/.dotfiles/bin/openlist param1=$file terminal=false color=$headercolor";
while read -r iid
      read -r labels
      read -r title
      read -r web_url; do
...
| href=$web_url font=$monofont size=$monosize";
echo $web_url >> $file;
done < <(jq -rc '.[] | .iid,.labels,.title,.web_url' < /tmp/gitlab-assignments-checker-5-1.json);

echo "---";
file=/tmp/gitlab-assignments-checker-1-standby.txt
> $file
echo "Dimi Standby | bash=/Users/dimitrie/.dotfiles/bin/openlist param1=$file terminal=false color=$headercolor";
while read -r iid
      read -r labels
      read -r title
      read -r web_url; do
...
| href=$web_url font=$monofont size=$monosize";
echo $web_url >> $file;
done < <(jq -rc '.[] | .iid,.labels,.title,.web_url' < /tmp/gitlab-assignments-checker-6-1.json);
