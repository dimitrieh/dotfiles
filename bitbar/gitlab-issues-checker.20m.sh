...
TPAGES=$(curl -i -s -H "PRIVATE-TOKEN: $privatetoken" "https://gitlab.com/api/v4/groups/9970/issues/?scope=all&state=opened&labels=Package&per_page=100" | grep -Fi X-Total-Pages | awk '/X-Total-Pages/ { print $2 }' | tr -d '\r');
...
for i in $(seq 1 $TPAGES); do
  curl -s -L -H "PRIVATE-TOKEN: $privatetoken" "https://gitlab.com/api/v4/groups/9970/issues/?scope=all&state=opened&labels=Package&state=opened&per_page=100&page=$i" | jq -rc '.[].updated_at |= (sub("\\....Z";"Z") | fromdate)' >> $allissues;
...
echo " PA $counttotal | templateImage=$gitlabicon";
...
$(printf %-75.75s "$title") | href=$web_url font=$monofont size=$monosize";
    echo $web_url >> $tfile;
  done < <(jq -rc ''"$2"' | .iid,.title,.web_url' < $issuesfile);
}
...
# Lists of epics
curl -s -L -H "PRIVATE-TOKEN: $privatetoken" "https://gitlab.com/api/v4/groups/9970/epics/?labels=Package&state=opened&order_by=updated_at&per_page=100" > /tmp/gitlab-issues-epics.json # Create issues file with 10 items
...
filter 'Sorted by upvotes (>25 upvotes)' 'sort_by(.upvotes) | reverse[] | select(.upvotes > 25)'
filter 'Sorted by comments (>25 comments)' 'sort_by(.user_notes_count) | reverse[] | select(.user_notes_count > 25)'
filter 'With bugs label' '.[] | select(.labels[]? == "bug")?'
filter 'With customer label' '.[] | select(.labels[]? == "customer")?'
