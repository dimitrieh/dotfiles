...
number1=$(($(cat /tmp/gitlab-todo-checker-1-1.json | /usr/local/bin/jq -rc '.[] | select(.target.assignee.username == "'$username'") | .target_url' | wc -l) + $(cat /tmp/gitlab-todo-checker-3-1.json | /usr/local/bin/jq -rc '.[] | select(.target.assignee.username == "'$username'") | .target_url' | wc -l)));
...
while read -r iid
    ...
    echo "#$(printf '%-6s' "$iid") $(printf %-15.15s "$path") $(printf %-75.75s "$title") | href=$target_url font=$monofont"
...
while read -r iid
      ...
      read -r target_url; do
    echo "#$(printf '%-6s' "$iid") $(printf %-15.15s "$path") $(printf %-75.75s "$title") | href=$target_url font=$monofont"
done < <(/usr/local/bin/jq -rc '.[]  | select(.target.assignee.username == "'$username'") | .target.iid,.project.path,.target.title,.target_url' < /tmp/gitlab-todo-checker-3-1.json);
echo "---";
