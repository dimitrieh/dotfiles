...
done < <(/usr/local/bin/jq -rc '.[]  | select(.target.assignee.username == "$username") | select(.target_type == "MergeRequest") |.target.iid,.project.path,.target.title,.target_url' < /tmp/gitlab-todo-checker-2-1.json)
echo "---"
