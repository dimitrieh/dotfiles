...
done < <(/usr/local/bin/jq -rc '.[]  | select(.target.assignee.username == "'$username'") | select(.target_type == "MergeRequest") | .target.iid,.project.path,.target.state,.target.title,.target_url' < /tmp/gitlab-todo-checker-2-1.json);
...
while read -r iid
      ...
      read -r target_url; do
    echo "$(printf %-15.15s "$path") #$(printf '%-6s' "$iid") $(printf %-75.75s "$([[ $state == *'opened'* ]] && echo '' || echo "("$state") ")$title") | href=$target_url font=$monofont"
done < <(/usr/local/bin/jq -rc '.[]  | select(.target.assignees[].username == "'$username'") | select(.target_type == "Issue") | .target.iid,.project.path,.target.state,.target.title,.target_url' < /tmp/gitlab-todo-checker-2-1.json);
...
while read -r iid
      ...
      read -r target_url; do
    echo "$(printf %-15.15s "$path") !$(printf '%-6s' "$iid") $(printf %-75.75s "$([[ $state == *'opened'* ]] && echo '' || echo "("$state") ")$title") | href=$target_url font=$monofont"
done < <(/usr/local/bin/jq -rc '.[] | select(.target_type == "MergeRequest") | .target.iid,.project.path,.target.state,.target.title,.target_url' < /tmp/gitlab-todo-checker-2-1.json);
