...
while read -r iid
      ...
      read -r target_url; do
    echo "#$(printf '%-6s' "$iid") $(printf %-15.15s "$path") $(printf %-75.75s "$title") | href=$target_url font=$monofont"
done < <(/usr/local/bin/jq -rc '.[]  | select(.target.assignee.username == "'$username'") | select(.target_type == "Issue") | .target.iid,.project.path,.target.title,.target_url' < /tmp/gitlab-todo-checker-2-1.json);
echo "---";
