...
number1=$(($(cat /tmp/gitlab-todo-checker-1-1.json | /usr/local/bin/jq -rc '.[] | select(.target_type == "MergeRequest") | select(.target.assignee.username == "'$username'") | .target_url' | wc -l) + $(cat /tmp/gitlab-todo-checker-3-1.json | /usr/local/bin/jq -rc '.[] | select(.target_type == "Issue") | select(.target.assignees[].username == "'$username'") | .target_url' | wc -l)));
...
while read -r iid
      ...
      read -r target_url; do
    echo "$(printf %-15.15s "$path") #$(printf '%-6s' "$iid") $(printf %-75.75s "$([[ $state == *'opened'* ]] && echo '' || echo "("$state") ")$title") | href=$target_url font=$monofont"
done < <(/usr/local/bin/jq -rc '.[]  | select(.target.assignees[].username == "'$username'") | .target.iid,.project.path,.target.state,.target.title,.target_url' < /tmp/gitlab-todo-checker-1-1.json);
...
while read -r iid
      ...
      read -r target_url; do
    echo "$(printf %-15.15s "$path") $([[ $target_url == *'merge_requests'* ]] && echo '!' || echo '#')$(printf '%-6s' "$iid") $(printf %-75.75s "$([[ $state == *'opened'* ]] && echo '' || echo "("$state") ")$title") | href=$target_url font=$monofont"
done < <(/usr/local/bin/jq -rc '.[] | select(.target.milestone.title == "10.3") | .target.iid,.project.path,.target.state,.target.title,.target_url' < /tmp/gitlab-todo-checker-1-1.json);
